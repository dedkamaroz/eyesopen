...
def annotate_image(image: np.ndarray, text: str, explanation: str) -> None:
    """
    Annotate an image with text and an optional explanation.
...
def convert_to_color(images: List[np.ndarray]) -> List[np.ndarray]:
    ...
    for img in images:
        if img is None:
            logging.warning("Encountered a NoneType image during color conversion. Skipping.")
            continue
